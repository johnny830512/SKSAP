...
class Utility_Method():
    ...
    def add_normal_grade(self, df):
        ...
        items = ['茶袋保持力', '0.7 psi AAP', '0.3psiAAP', '假比重']
        #品別
        grade = ['BC283FA','BC283FAN','BC283GAN','BC583AN','BC586G'] # 高中和度
        #???
        plant_unit = ['SKS_SXFU']
        ...
        lims_df = lims_obj.get_lims(lims_cols, sample_points, items, grade, 
                             start_time, time)
        GRADENAME = pd.DataFrame(columns = ['BC283FA','BC283FAN','BC283GAN','BC583AN','BC586G'] , data = [[0,0,0,0,0]])
        GRADENAME.reindex(columns = ['BC283FA','BC283FAN','BC283GAN','BC583AN','BC586G'])
        try:
            GRADENAME[lims_df['GRADENAME'][len(lims_df)-1]] = 1  #嘗試把最近一次的品別納入訓練 
        except:
            pass #撈不到就跳過
